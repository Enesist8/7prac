...
class InventoryController {
    ...
    async getOneInventoryItem(req, res) {
        ...
        try {
            const client = await db.connect();
            const result = await client.query('SELECT * FROM inventory WHERE inventory_id = $1', [id]);
            client.release();
    ...
    async deleteInventoryItem(req, res) {
        ...
        try {
            const client = await db.connect();
            const result = await client.query('DELETE FROM inventory WHERE inventory_id = $1', [id]);
            client.release();
