...
class InstrumentTypeController {
    ...
    async getOneInstrumentType(req, res) {
        ...
        try {
            const client = await db.connect();
            const result = await client.query('SELECT * FROM instrument_types WHERE type_id = $1', [id]);
            client.release();
    ...
    async updateInstrumentType(req, res) {
        ...
        try {
            ...
            return res.status(500).json({ error: 'Failed to update instrument type' });
        }
    }
    ...
    async deleteInstrumentType(req, res) {
        ...
        try {
            const client = await db.connect();
            const result = await client.query('DELETE FROM instrument_types WHERE type_id = $1', [id]);
            client.release();
